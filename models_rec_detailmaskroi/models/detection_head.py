...
from .losses.selfm_loss import selfm_loss
from .pa_pyx import pa

from models.extra.ThresholdMap import AddThreshold

add_kernel = AddThreshold(in_channels=1).to("cuda")
...
class DetectionHead(nn.Module):
    ...
    def loss(self,
        ...
        # print("loss------------------------------------------")
        # print("loss: canny", cannys.shape)
        # print("loss: fk", feature_kernel.shape)
        ...
        if feature_kernel != None:

            kernels = add_kernel(kernels, feature_kernel)

            loss_detail = selfm_loss(feature_kernel, cannys, selected_masks, reduce=False)

            # print("loss_detail", loss_detail)

            # loss_selfm = selfm_loss(kernels, texts, gt_texts, selected_masks, reduce=False)

        embs = out[:, 2:, :, :]

        # add point --------------------------------------------------------------------------------------------------20230321

        loss_text = dice_loss(texts, gt_texts, selected_masks, reduce=False) + loss_detail
