...
class FPEM(nn.Module):
    def __init__(self, planes):
        ...
        self.fuse_conv_block = nn.Conv2d(
            planes * 2,
            planes,
            kernel_size=3,
            stride=1,
    ...
    def forward(self, f1, f2, f3, f4, f1_o, f2_o, f3_o, f4_o):
        ...
        # print("before f1.shape:",f1.shape)

        f1 = self.fuse_conv_block(torch.cat((f1, f1_o), 1))
        f2 = self.fuse_conv_block(torch.cat((f2, f2_o), 1))
        f3 = self.fuse_conv_block(torch.cat((f3, f3_o), 1))
        f4 = self.fuse_conv_block(torch.cat((f4, f4_o), 1))

        # print("after -- f1.shape:", f1.shape)

        return f1, f2, f3, f4


from models.CA_Module import CoordAttention
...
class ResNet(nn.Module):
    ...
    def __init__(
        ...
        # add channel attention module ------------------------------------------------------------------------20230324
        self.se_module_ext = CoordAttention(in_channels=128, out_channels=128)

        hidden_dim = 128

        # watch layer.shape ---------------------------------------------------------------------------------2023-03-16
        # for layer in block.expansion:
        #     print(layer)
        ...
        # feature_kernel-------------------------------------------------------------------------------------20230321 uoqu zui tezheng de  kernel jisuan dice loss
        # self.feature_kernel=
    ...
    def forward(self,
        ...
        # imporvd point -----------------------------------------------------------------2023-03-16

        h = self.layer1(h)

        f1 = self.reduce_layer1(h)
        f1_reduce_origin = self.se_module_ext(self.reduce_layer1(h))
        feature_kernel_1 = self.mp(f1_reduce_origin)

        h = self.layer2(h)
        f2 = self.reduce_layer2(h)
        f2_reduce_origin = self.se_module_ext(self.reduce_layer2(h))

        h = self.layer3(h)
        f3 = self.reduce_layer3(h)
        f3_reduce_origin = self.se_module_ext(self.reduce_layer3(h))

        h = self.layer4(h)
        f4 = self.reduce_layer4(h)
        f4_reduce_origin = self.se_module_ext(self.reduce_layer4(h))
        ...
        # FPEM
        f1, f2, f3, f4 = self.fpem1(f1, f2, f3, f4, f1_reduce_origin, f2_reduce_origin, f3_reduce_origin,
                                    f4_reduce_origin)
        feature_kernel_2 = self.mp(self.se_module_ext(f1))
        f1, f2, f3, f4 = self.fpem2(f1, f2, f3, f4, f1_reduce_origin, f2_reduce_origin, f3_reduce_origin,
                                    f4_reduce_origin)
        feature_kernel_3 = self.mp(self.se_module_ext(f1))

        f2 = self._upsample(f2, f1)
        f3 = self._upsample(f3, f1)
        f4 = self._upsample(f4, f1)

        f2_reduce_origin = self._upsample(f2_reduce_origin, f1)
        f3_reduce_origin = self._upsample(f3_reduce_origin, f1)
        f4_reduce_origin = self._upsample(f4_reduce_origin, f1)

        f = torch.cat((f1 + f1_reduce_origin, f2 + f2_reduce_origin, f3 + f3_reduce_origin, f4 + f4_reduce_origin), 1)

        hidden_dim = 128

        feature_kernel_4 = self.mp(self.se_module_ext(f[:, 0:hidden_dim]))

        feature_kernel = torch.cat((feature_kernel_1, feature_kernel_2, feature_kernel_3, feature_kernel_4), dim=1)
        ...
        out_feature_kernel = self.detection_head(
            feature_kernel,
            (imgs.size(2) // self.scale, imgs.size(3) // self.scale))
        #
        # print(feature_kernel.shape, "  feature_kernel-------------total")
        # print("out_det", out_det.shape)
        # print("out_feature_kernel", out_feature_kernel.shape)
        ...
        if self.training:
            loss_det = self.detection_head.loss(
                ...
                training_masks,
                gt_instances,
                gt_bboxes, args=args, feature_kernel=out_feature_kernel)
        ...
        # iou_text = outputs['iou_text']
        # loss_kernels = outputs['loss_kernels']
        # iou_kernel = outputs['iou_kernel']
        # loss_dis = outputs['loss_dis']
        #
        # print("loss_text", loss_text.shape)
        # print(iou_text.shape)
        # print(loss_kernels.shape)
        # print(iou_kernel.shape)
