...
class PPLCNet(nn.Module):
    def __init__(self, scale=1.0, num_classes=1000, dropout_prob=0.2):
        ...
        # print(layers)
        self.features = nn.Sequential(*layers[0:11])

        self.conv_11 = nn.Sequential(*layers[11:12])
        self.conv_12 = nn.Sequential(*layers[12:13])
        self.conv_13 = nn.Sequential(*layers[13:14])
        self.conv_14 = nn.Sequential(*layers[14:15])

        self.conv_up_h_swish = nn.Sequential(
            nn.ConvTranspose2d(input_channel, input_channel // 2, 3, 2, 1, output_padding=1, bias=False),
            nn.BatchNorm2d(input_channel // 2),
            HardSwish()
        )
        self.conv_up_h_swish4 = nn.Sequential(
            nn.ConvTranspose2d(input_channel // 2, input_channel // 4, 3, 2, 1, output_padding=1, bias=False),
            nn.BatchNorm2d(input_channel // 4),
            HardSwish()
        )

        self.conv_down_h_swish = nn.Sequential(
            nn.Conv2d(input_channel, input_channel // 2, 3, 1, 1, bias=False),
            nn.BatchNorm2d(input_channel // 2),
            HardSwish()
        )

        self.conv_down_h_swish4 = nn.Sequential(
            nn.Conv2d(input_channel // 2, input_channel // 4, 3, 1, 1, bias=False),
            nn.BatchNorm2d(input_channel // 4),
            HardSwish()
        )

        self.conv_128to64 = nn.Sequential(
    ...
    def forward(self, x):
        ...
        x_range0_11 = self.features(x)

        x_feature_11 = self.conv_11(x_range0_11)
        x_feature_12 = self.conv_12(x_feature_11)
        x_feature_13 = self.conv_13(x_feature_12)
        x_feature_14 = self.conv_14(x_feature_13)

        # print(x_feature_12.shape, "12 shape")

        x_feature_up_s46 = self.conv_up_h_swish(x_feature_14)
        x_feature_13 = self.conv_down_h_swish(torch.cat((x_feature_13, x_feature_up_s46), dim=1))

        x_feature_12and13_up = self.conv_up_h_swish(torch.cat((x_feature_13, x_feature_12), dim=1))

        # print("x_feature_12and13_up", x_feature_12and13_up.shape)
        x_feature_up_s96 = self.conv_down_h_swish4(x_feature_12and13_up)

        x_feature_12 = x_feature_up_s96

        x_feature_11 = self.conv_128to64(x_feature_up_s96)

        x = self.avgpool(x_feature_14)
        ...
        return [x_feature_11, x_feature_12, x_feature_13, x_feature_14]
