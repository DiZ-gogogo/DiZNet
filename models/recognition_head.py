...
class RecognitionHead(nn.Module):
    ...
    def extract_feature(self, f, output_size, instance, bboxes, gt_words=None,
                        word_masks=None, unique_labels=None, feature_kernel=None, out_det=None):

        fk = feature_kernel.squeeze()
        instance = fk.long() + instance.long()

        random_channel_index = torch.randint(0, 6, (1,))
        select_channel = out_det[:, random_channel_index, :, :]
        select_channel = self.relu(select_channel / 255)
    ...
    def forward(self, x, holistic_feature, target):
        ...
        for t in range(max_seq_len + 1):
            ...
            for i in range(self.num_layers):
                ...
                # print("lstm inp",inp.shape)
                h[i] = self.lstm_u[i](inp, h[i])
                # print("lstm out",h[i].shape)
            ht = h[-1][0]
            # print("lstm out", ht.shape)
...
class MultiHeadAttentionLayer(nn.Module):
    def __init__(self, hidden_dim, n_heads, dropout=0.1):
        ...
        # self.t_w = nn.Parameter(torch.ones(n_heads, 7, 7))
